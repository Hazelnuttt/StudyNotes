...
- redux 数据的生命周期
  ...
  1. 调用 `store.dispatch(action)` --> (触发 action)
  2. Redux Store 调用 reducer 函数 --> (store 会把两个参数传入 reducer ：当前的 state 和 action)
  3. 根 reducer 把多个 子 reducer 输出合并成一个单一的 state 树 --> (是否有多个 reducer，视情况。所有 state 都以一个对象树的形式合并存储是一定的。)
